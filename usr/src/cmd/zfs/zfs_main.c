	...
	if (do_all) {
		...
		while (--nitems >= 0) {
			if (strcmp(sfsp->f_fstypename, MNTTYPE_ZFS) != 0 ||
			    strchr(sfsp->f_mntfromname, '@') != NULL) {
				++sfsp;
				continue;
			}
...
#ifdef __APPLE__
	/*
	 * Search for the given device in the mount table.
	 */
	if ((nitems = getmntinfo(&sfsp, MNT_WAIT)) == 0) {
		return (1);
	}
...
unshare_unmount(int op, int argc, char **argv)
{
	...
	if (do_all) {
		...
		while (getmntent(mnttab_file, &entry) == 0) {
			...
			if ((node->un_mountp = strdup(sfsp->f_mntonname)) ==
			    NULL) {
				(void) fprintf(stderr, gettext("internal error:"
				    " out of memory\n"));
				exit(1);
...
#ifdef __APPLE__
		if (mntopts[0])
			printf("manual_mount: unused options: \"%s\"\n", mntopts);
		if (mount(MNTTYPE_ZFS, path, flags, dataset) != 0) {
#else
