...
zfs_prop_init(void)
{
...
#ifdef __APPLE__
	/* Table for properties which are unsupported on Mac OSX */
	static zprop_index_t notsup_table[] = {
		{ "off",	0 },
		{ NULL }
	};
#endif
	...
	register_index(ZFS_PROP_READONLY, "readonly", 0, PROP_INHERIT,
	    ...
	    boolean_table);
	/* Mac OSX doesn't support the zoned property*/
	register_index(ZFS_PROP_ZONED, "zoned", 0, PROP_INHERIT,
	    ZFS_TYPE_FILESYSTEM, "off", "ZONED", notsup_table);
...
#ifdef __APPLE__
	/* XXX- Mac OSX will not support the autoreplace and delegated 
	 * administration properties on pools for now.  We will work on 
	 * bringing them in, in the future
	 */
	register_index(ZPOOL_PROP_DELEGATION, "delegation", 0, PROP_DEFAULT,
	    ZFS_TYPE_POOL, "off", "DELEGATION", notsup_table);
	register_index(ZPOOL_PROP_AUTOREPLACE, "autoreplace", 0, PROP_DEFAULT,
	    ZFS_TYPE_POOL, "off", "REPLACE", notsup_table);
#else
	...
	register_string(ZFS_PROP_SHAREISCSI, "shareiscsi", "off", PROP_INHERIT,
	    ZFS_TYPE_DATASET, "off", "SHAREISCSI");
#else
	register_string(ZFS_PROP_SHARENFS, "sharenfs", "off", PROP_INHERIT,
