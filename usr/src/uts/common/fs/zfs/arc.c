...
#ifdef __APPLE__
static uint64_t         arc_c_peak;     /* Peak cache size */
#endif
...
buf_hash(spa_t *spa, dva_t *dva, uint64_t birth)
{
	...
	for (i = 0; i < sizeof (dva_t); i++)
		crc = (crc >> 8) ^ zfs_crc64_table[(crc ^ vdva[i]) & 0xFFULL];
...
buf_init(void)
{
	...
	/*
	 * The hash table is big enough to fill half of physical memory
	 * with an average 64K block size.  The table will take up
	 * totalmem*sizeof(void*)/64K (eg. 128KB/GB with 8-byte pointers).
	 */
	while (hsize * (65536/2) < physmem * PAGESIZE)
		hsize <<= 1;
retry:
	buf_hash_table.ht_mask = hsize - 1;
	buf_hash_table.ht_table =
#ifdef __APPLE__
	    kmem_zalloc(hsize * sizeof (void*), KM_SLEEP);
#else
...
#ifdef __APPLE__
	/*
	 * Start reclaiming when our footprint bloats.
	 */
...
arc_kmem_reap_now(arc_reclaim_strategy_t strat)
{
	 ...
	 */
	kmem_cache_reap_now(znode_cache_get());
	kmem_cache_reap_now(dnode_cache_get());
	kmem_cache_reap_now(dbuf_cache_get());
#endif /* __APPLE__ */
}
...
top:
	/* 
	 * Note: buf_has_find will return with the hash lock held if he finds
	 * the buffer
	 */
...
#ifdef __APPLE__
void
arc_get_stats(zfs_memory_stats_t *stats)
{
	stats->current = arc_size;
	stats->target = arc_c;
	stats->highest = arc_c_peak;
	stats->maximum = arc_c_max;
}
#endif /* __APPLE__ */
